...
sub writeSampleSummary{
    ...
    foreach my $s (
        ...
        my $row = 1;
        my ($workbook, $worksheet) = setupOutput($s);
        #rank on damaging consequence category for this sample
        @{$sample_vars{$s}} = 
            sort {
                $b->[0] <=> $a->[0] 
            } @{$sample_vars{$s}};
        foreach my $var (@{$sample_vars{$s}}){
...
##############ADD VALUES TO ROW FROM SPREADSHEET##
    ...
    foreach my $f (reverse(@vep_fields) ){
        unshift @row, $csq_to_report->{$f};
    }
    ...
    foreach my $s (
        ...
        if (grep { $_ eq $var->{ORIGINAL_ALT} } @alts ){ 
            ...
            my @sample_cells = ( $samp_gts{$s}, join(",", @ads) , $ab, $samp_gqs{$s}, );
            {
                no warnings 'uninitialized';
                push @{$sample_vars{$s}}, [$allele_cadd, @row, @sample_cells, ];
            }
        }
    }
}
...
sub setupOutput{
    ...
    my $worksheet = $xl->add_worksheet("Variants"); 
    $header_formatting = $xl->add_format(bold => 1);
    writeHeader($worksheet, $sheets{Variants}, $headers{mostdamaging});
    my $col = 0;
    foreach my $h (@{$headers{mostdamaging}}){
        $worksheet->write(0, $col++, $h, $header_formatting);
    }
    return ($xl, $worksheet);
}
...
=cut
    ...
    $md_spl++ until $h{mostdamaging}->[$md_spl] eq 'Chrom'; 
    ...
    if (exists $search_handles{et}){
        ...
        splice (@{$h{mostdamaging}}, -5, 0,  "EvolutionaryTraceScore");
    }
    return %h;
}
###########################################################
sub writeHeader{
    ...
    my $col = 0;
    foreach my $h (@$header){
        $xl->write(0, $col++, $h, $header_formatting);
    }
}

###########################################################
sub addSheet{
    my ($xl_obj, $name, $header) = @_; 
    my $sheet = $xl_obj->addWorksheet($name);
    writeHeader($xl_obj, $sheet, $header);
    return $sheet;
}
